...
const AllFlightsPage = () => {
  ...
  useEffect(() => {
    ...
    axios
     .get('https://airline-management-wzaz.onrender.com/api/flight/getAllflights', {
        headers: {
          Authorization: token,
  ...
  const handleDeleteFlight = async (flightId) => {
    ...
    const response  = await axios.get(`https://airline-management-wzaz.onrender.comapi/flight/deleteflight/${flightId}`, {
        headers: {
          Authorization: token,
        },
      })
  ...
  const handleFormSubmit = async (formData) => {
    try {
      ...
      const response = await axios.post('https://airline-management-wzaz.onrender.com/api/flight/createflight', formData, {
        headers: {
          Authorization: token,
          'Content-Type': 'application/json',
        },
